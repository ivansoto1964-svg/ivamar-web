...
const primaryArtisanContact = item => {
  ...
  if (/^[0-9]{10,15}$/.test(phone)) return {href:`https://wa.me/${phone}`,label:'Contactar al artesano',track:'whatsapp'};
...
function artesanoPerfil(item, helpers) {
  ...
  const social = [];
  const whatsapp = String(item.whatsapp || '').replace(/[^0-9]/g,'');
  if (/^[0-9]{10,15}$/.test(whatsapp)) social.push(`<a class="btn wa" data-pb-track="whatsapp" href="https://wa.me/${whatsapp}" target="_blank" rel="ugc nofollow noopener noreferrer">Contactar por WhatsApp</a>`);
...
module.exports = function artesanoPerfilConResponsable(item, helpers) {
  ...
  const qrDownloadUrl = `/artesanos/${encodeURIComponent(helpers.slug)}/qr.png`;
  html = html.replace(
    '.share{text-align:center;margin:2rem 0}.share a{color:#ce1126;font-weight:700}',
    '.share{margin:1.1rem 0 1.3rem;padding:1.25rem;background:#fff;border:1px solid #dfe4eb;border-radius:12px;text-align:center}.share h2{margin:0;color:#002d62;font-size:1.25rem}.share p{margin:.35rem 0 1rem;color:#596273;line-height:1.5}.share-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem}.share-action{display:inline-flex;align-items:center;justify-content:center;min-height:46px;border:1px solid #cbd4df;border-radius:8px;background:#fff;color:#002d62;padding:.72rem .75rem;font:800 .82rem Inter,system-ui,sans-serif;text-decoration:none;cursor:pointer}.share-action.primary{background:#002d62;color:#fff;border-color:#002d62}.share-action.facebook{background:#1877f2;color:#fff;border-color:#1877f2}.share-action.instagram{background:linear-gradient(135deg,#833ab4,#fd1d1d,#fcb045);color:#fff;border-color:#c13584}.share-action.whatsapp{background:#178c49;color:#fff;border-color:#178c49}.share-action.qr{background:#ce1126;color:#fff;border-color:#ce1126}.share-action:focus-visible{outline:3px solid #ce1126;outline-offset:3px}.share-status{min-height:1.2em;margin:.75rem 0 0!important;color:#36516e!important;font-size:.8rem}@media(max-width:700px){.share-actions{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:420px){.share-actions{grid-template-columns:1fr}.share-action{width:100%}}'
  );
  html = html.replace(
  ...
  const shareSection = html.match(/<section class="share"[\s\S]*?<\/section>/)?.[0];
  if (shareSection) {
    html = html.replace(shareSection, '').replace('</article>', `</article>${shareSection}`);
  }
  html = html.replace(
