...
const areaOf = event => event.virtual
  ? 'virtual'
  : /^(puerto rico|pr)$/i.test(event.country || '') || /^puerto rico$/i.test(event.region || '')
    ? 'pr'
    : 'usa';
...
function eventCards(events) {
  return events.map(event => {
    ...
    const month = date.toLocaleDateString('es-PR', { month:'short' }).replace('.', '');
    ...
    const image = event.image || '/img/agenda-boricua-placeholder.svg';
    const detailPath = eventPath(event);
    const whatsapp = `https://wa.me/?text=${encodeURIComponent(`${event.name} https://www.masboricuaqueunmofongo.com${detailPath}`)}`;
    return `<article class="event-card" data-area="${areaOf(event)}" data-region="${esc(region).toLowerCase()}" data-city="${esc(event.city).toLowerCase()}"><a class="event-image" href="${esc(detailPath)}"><img src="${esc(image)}" alt="Afiche de ${esc(event.name)}" loading="lazy"></a><div class="event-content"><div class="date"><strong>${date.getDate()}</strong><span>${esc(month)}</span></div><div class="event-copy"><div class="type">${esc(event.type || 'Evento cultural')}</div><h2><a href="${esc(detailPath)}">${esc(event.name)}</a></h2><p class="where">📍 ${esc(location)}</p>${event.time ? `<p><strong>Horario:</strong> ${esc(event.time)}</p>` : ''}<p>${esc(event.description)}</p><div class="presenter">${presenter}</div><div class="actions"><a href="${esc(detailPath)}">Ver evento</a>${event.eventUrl ? `<a href="${esc(event.eventUrl)}" target="_blank" rel="noopener">Información oficial</a>` : ''}<a href="${esc(whatsapp)}" target="_blank" rel="noopener">Compartir</a></div></div></div></article>`;
  }).join('');
}

function eventSchema(event) {
  const siteUrl = 'https://www.masboricuaqueunmofongo.com';
  const detailUrl = `${siteUrl}${eventPath(event)}`;
  const officialUrl = /^https?:\/\//i.test(String(event.eventUrl || '')) ? event.eventUrl : '';
  const image = event.image
    ? (/^https?:\/\//i.test(event.image) ? event.image : `${siteUrl}${event.image.startsWith('/') ? '' : '/'}${event.image}`)
    : `${siteUrl}/img/agenda-boricua-placeholder.svg`;
  const organizerName = event.organizerName || event.sourceLabel || event.artisanName || '';
  return {
