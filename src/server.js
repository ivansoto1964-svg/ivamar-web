...
<textarea name="description" rows="4" placeholder="Describe tu negocio" style="margin-bottom:16px;padding:10px;"></textarea>
      ...
      <label>Acompañantes</label>
      <textarea name="sides" rows="4" placeholder="Ej. Papas fritas - $4&#10;Tostones - $5&#10;Ensalada - $3" style="margin-bottom:16px;padding:10px;"></textarea>

<label>Bebidas</label>
<textarea name="drinks" rows="4" placeholder="Ej. Refresco - $2&#10;Agua - $1&#10;Malta - $3" style="margin-bottom:16px;padding:10px;"></textarea>

<label>Postres</label>
<textarea name="desserts" rows="4" placeholder="Ej. Flan - $4&#10;Tres leches - $5&#10;Brownie - $3" style="margin-bottom:16px;padding:10px;"></textarea>
      ...
      <label>Logo del negocio (link)</label>
      <input type="text" name="logoUrl" placeholder="Pega aquí el link de tu logo o escribe pendiente" style="margin-bottom:16px;padding:10px;" />

      <label>Foto principal del negocio (link)</label>
      <input type="text" name="businessPhotoUrl" placeholder="Ej. foto del food truck, local, plato estrella o frente del negocio" style="margin-bottom:16px;padding:10px;" />
...
<label>Estilo visual de tu página</label>
      ...
      <h2 style="margin-top:24px;">Tu asistente digital</h2>
      <p style="margin-bottom:20px;">
        Personaliza el nombre y estilo del asistente que verán tus clientes.
      </p>
      ...
      <label>Palabras o frases clave que usa mucho</label>
      <input type="text" name="assistantKeywords" placeholder="Ej. wepa, duro, al punto" style="margin-bottom:16px;padding:10px;" />

      <label>Avatar o imagen del asistente (link)</label>
      <input type="text" name="assistantAvatarUrl" placeholder="Pega aquí el link de la imagen" style="margin-bottom:16px;padding:10px;" />
...
const {
  ...
  while (
    clients.some(item => item.slug === finalSlug) ||
    fs.existsSync(path.join(__dirname, "..", "data", "businesses", `${finalSlug}.json`))
  ) {
...
const parsedMenu = (menu || "")
  ...
  .map(line => {
    ...
    const hyphenMatch = line.match(/^(.*?)-\s*\$?(\d+(?:\.\d{1,2})?)$/);
    const endPriceMatch = line.match(/^(.*)\s+(\d+(?:\.\d{1,2})?)$/);

    if (hyphenMatch) {
      itemName = (hyphenMatch[1] || "").trim();
      price = Number(hyphenMatch[2]);
    } else if (endPriceMatch) {
      itemName = (endPriceMatch[1] || "").trim();
      price = Number(endPriceMatch[2]);
    } else {
      itemName = line;
      desc = line;
    }
    ...
    return {
      ...
      desc: Number.isFinite(price) ? "" : desc
    };
  });

const parsedSides = (sides || "")
  .split("\n")
  .map(line => line.trim())
  .filter(Boolean)
  .map(line => {
...
const businessData = {
  ...
  headline: `${newClient.businessType || "Negocio local"} con Ivamar AI`,
  description: description || "Próximamente más información sobre este negocio.",
  logoUrl: newClient.logoUrl || "",
  businessPhotoUrl: newClient.businessPhotoUrl || "",
  address: newClient.address || "",
  hours: hours || "",
  status: status || "abierto",
  theme: newClient.theme || "light",

  assistant: {

    tone: newClient.assistantTone || "amistoso y profesional",
    welcome: newClient.assistantWelcome || `Hola, soy ${newClient.assistantName || "IvA"}. ¿En qué te ayudo hoy?`,
    keywords: (newClient.assistantKeywords || "")
      .split(",")
      .map(word => word.trim())
      .filter(Boolean),
    avatarUrl: newClient.assistantAvatarUrl || ""
  },
  links: {
    whatsapp: newClient.whatsapp
      ? `https://wa.me/${String(newClient.whatsapp).replace(/\D/g, "")}`
      : "#",
    instagram: "#",
    googleMaps: "#"
  },

menu: parsedMenu.length
  ? parsedMenu
  : [
      {
        name: "Menú pendiente",
        price: 0,
        desc: "Este negocio todavía no ha publicado su menú completo."
      }
    ],
sides: parsedSides.length ? parsedSides : [],
drinks: parsedDrinks.length ? parsedDrinks : [],
desserts: parsedDesserts.length ? parsedDesserts : [],
...
fs.writeFileSync(businessFile, JSON.stringify(businessData, null, 2), "utf8");
  ...
  try {
    const out = await postJson(
      BILLING_API_URL,
      {
        ...
        successUrl: `${siteUrl}/${newClient.slug}`,
    ...
    let data = null;
    try {
      data = out.body ? JSON.parse(out.body) : null;
    } catch (_) {
      data = null;
    }
    ...
    if (out.status >= 200 && out.status < 300 && checkoutUrl) {
  ...
  return res.redirect(`/${newClient.slug}`);
});
...
app.get("/:slug", (req, res) => {
  ...
  // 1) Si existe negocio completo en data/businesses, usar ese primero
  ...
  if (fs.existsSync(businessFile)) {
    ...
    let pageStyle = "";
    if (theme === "dark") {
      pageStyle = "background:#111;color:#f5f5f5;";
    } else if (theme === "tropical") {
      pageStyle = "background:linear-gradient(180deg,#fff8e7 0%,#ffe8c2 100%);color:#2b1d0e;";
    } else {
      pageStyle = "background:#ffffff;color:#111111;";
    }
...
if (data.status === "cerrado") {
  statusLabel = "🔴 Cerrado";
} else if (data.status === "vacaciones") {
  statusLabel = "🌴 Vacaciones";
} else if (data.status === "feriado") {
  statusLabel = "📅 Feriado";
} else {
  statusLabel = "🟢 Abierto";
}
...
<div style="text-align:center;margin-bottom:18px;">
  <div style="display:inline-block;margin-bottom:10px;padding:8px 14px;border-radius:999px;background:rgba(255,255,255,.08);font-weight:700;">
    ${statusLabel}
  </div>
  <h1 style="margin:20px 0 8px 0;font-size:34px;">${data.name}</h1>
  <p style="margin:0 auto;max-width:700px;color:var(--muted);font-size:17px;">
    ${data.description || "Próximamente más información sobre este negocio."}
  </p>
</div>
...
<div style="margin:24px 0 18px 0;text-align:center;">
        ...
        <div class="card" style="margin-top:16px;">
          <h3 style="margin-top:0;">Habla con ${assistantName}</h3>
          <p style="margin-bottom:0;">${assistantWelcome}</p>
        </div>
        ...
        <h2 style="margin-top:40px;font-size:28px;">Menú</h2>
        <p style="color:#aaa;margin-top:4px;">Lo que tenemos disponible hoy</p>
        <div class="grid">
          ${(data.menu || []).map((item, index) => `
            <div class="tile">
              <b>${item.name}</b>
              <div>${item.desc && item.desc !== item.name ? item.desc : ""}</div>
              <div style="margin-top:8px;font-weight:900;color:var(--text)">
                ${item.price !== null ? `$${item.price}` : "Precio pendiente"}
              </div>

              <div style="margin-top:12px;display:flex;align-items:center;gap:10px;">
                <button type="button" onclick="changeQty('menu-${index}', -1)" style="padding:6px 10px;border:none;border-radius:8px;cursor:pointer;">-</button>
                <span id="qty-menu-${index}">0</span>
                <button type="button" onclick="changeQty('menu-${index}', 1)" style="padding:6px 10px;border:none;border-radius:8px;cursor:pointer;">+</button>
              </div>
            </div>
          `).join("")}
        ...
        ${(data.sides || []).length ? `
          <h2 style="margin-top:40px;font-size:28px;">Acompañantes</h2>
          <p style="color:#aaa;margin-top:4px;">Perfectos para completar tu orden</p>
          ...
          <div class="grid">
            ${(data.sides || []).map((item, index) => `
              <div class="tile">
                ...
                <div style="margin-top:12px;display:flex;align-items:center;gap:10px;">
                  <button type="button" onclick="changeQty('side-${index}', -1)">-</button>
                  <span id="qty-side-${index}">0</span>
                  <button type="button" onclick="changeQty('side-${index}', 1)">+</button>
                </div>
              </div>
            `).join("")}
        ...
        ${(data.drinks || []).length ? `
          <h2 style="margin-top:40px;font-size:28px;">Bebidas</h2>
          <p style="color:#aaa;margin-top:4px;">Para acompañar tu orden</p>
          ...
          <div class="grid">
            ${(data.drinks || []).map((item, index) => `
              <div class="tile">
                ...
                <div style="margin-top:12px;display:flex;align-items:center;gap:10px;">
                  <button type="button" onclick="changeQty('drink-${index}', -1)">-</button>
                  <span id="qty-drink-${index}">0</span>
                  <button type="button" onclick="changeQty('drink-${index}', 1)">+</button>
                </div>
              </div>
            `).join("")}
        ...
        ${(data.desserts || []).length ? `
          <h2 style="margin-top:40px;font-size:28px;">Postres</h2>
          <p style="color:#aaa;margin-top:4px;">Un toque dulce para terminar</p>
          ...
          <div class="grid">
            ${(data.desserts || []).map((item, index) => `
              <div class="tile">
                ...
                <div style="margin-top:12px;display:flex;align-items:center;gap:10px;">
                  <button type="button" onclick="changeQty('dessert-${index}', -1)">-</button>
                  <span id="qty-dessert-${index}">0</span>
                  <button type="button" onclick="changeQty('dessert-${index}', 1)">+</button>
                </div>
              </div>
            `).join("")}
...
<div id="cart" style="margin-top:40px;padding:20px;border-radius:20px;background:#f0f0f0;">
  ...
  <div id="cart-items" style="margin-bottom:16px;color:#555;">
...
<div style="margin-top:16px;font-weight:700;color:#222;">
  <div>Subtotal: $<span id="subtotal" style="color:#111;">0.00</span></div>
  <div>Tax: $<span id="tax" style="color:#111;">0.00</span></div>
  <div style="margin-top:8px;font-size:20px;font-weight:900;color:#000;">
    Total: $<span id="total" style="color:#000;">0.00</span>
  </div>
</div>


  style="margin-top:16px;width:100%;padding:12px;border:none;border-radius:10px;background:#25D366;color:white;font-weight:700;">
  Enviar pedido por WhatsApp
...
function changeQty(id, delta) {
  ...
  let qty = parseInt(el.textContent || "0", 10);
  qty += delta;
  if (qty < 0) qty = 0;
...
function updateCart() {
  ...
  function processItems(items, prefix) {
    items.forEach((item, index) => {
      const qtyEl = document.getElementById("qty-" + prefix + "-" + index);
      ...
      if (qty > 0) {
        ...
        subtotal += lineTotal;
        lines.push(item.name + " x" + qty + " = $" + lineTotal.toFixed(2));
      }
    });
  }

  processItems(menuItems, "menu");
  processItems(sidesItems, "side");
  processItems(drinksItems, "drink");
  processItems(dessertsItems, "dessert");

  if (lines.length === 0) {
    cartItemsEl.innerHTML = "No has añadido nada todavía.";
  } else {
    cartItemsEl.innerHTML = lines.join("<br/>");
  }
...
updateCart();
  ...
  // 2) Si no existe archivo en businesses, buscar en clients.json
  let clients = [];
  try {
    const raw = fs.readFileSync(clientsFilePath, "utf8");
    clients = JSON.parse(raw);
  ...
  if (client) {
    const body = `
      <div class="card">
        <h1>${client.businessName}</h1>
        <p><b>Tipo de negocio:</b> ${client.businessType || "No definido"}</p>
        <p><b>Descripción:</b> ${client.description || "Pendiente"}</p>
        <p><b>Contacto:</b> ${client.ownerName || "Pendiente"}</p>
        <p><b>Email:</b> ${client.email || "Pendiente"}</p>
        <p><b>WhatsApp:</b> ${client.whatsapp || "Pendiente"}</p>
        <p><b>Slug:</b> /${client.slug}</p>

        <div class="card" style="margin-top:16px;">
          <p><b>Menú / servicios:</b></p>
          <pre style="white-space:pre-wrap;font-family:inherit;">${client.menu || "Pendiente"}</pre>
        </div>

        <div class="btns" style="margin-top:16px;">
          <a class="btn primary" href="/start">Crear otro negocio</a>
          <a class="btn ghost" href="/">Volver</a>
        </div>
      </div>
    `;
