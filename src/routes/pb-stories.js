...
function renderStory(item) {
  ...
  const town = townName(item.town);
  const inviteText = `${item.name} compartió “${item.title}”, una historia de ${town}, en Planeta Boricua 🇵🇷\n\nLéela y comparte tu propio recuerdo o versión de nuestro pueblo:\n${canonical}`;
  const share = encodeURIComponent(inviteText);
  const paragraphs = item.story.split(/\n{2,}/).map(p=>`<p>${esc(p).replace(/\n/g,'<br>')}</p>`).join('');
  const place = item.location ? ` · ${esc(item.location)}` : '';
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(item.title)} — Historias de ${esc(town)} | Planeta Boricua</title><meta name="description" content="${esc(item.story.slice(0,155))}"><link rel="canonical" href="${canonical}"><meta property="og:type" content="article"><meta property="og:title" content="${esc(item.title)}"><meta property="og:description" content="Una historia de ${esc(town)} compartida en Planeta Boricua. Léela y comparte la tuya."><meta property="og:url" content="${canonical}"><meta property="og:image" content="https://www.masboricuaqueunmofongo.com/img/pb-logo.png"><style>*{box-sizing:border-box}body{margin:0;background:#f6f4ee;color:#1f1f1c;font-family:Inter,system-ui,sans-serif;line-height:1.7}nav{background:#fff;border-bottom:3px solid #ce1126;padding:1rem 5%;display:flex;justify-content:space-between;gap:1rem;flex-wrap:wrap}nav a{color:#002d62;text-decoration:none;font-weight:900}.hero{background:linear-gradient(135deg,#002d62,#184b80);color:#fff;padding:4rem 1rem;text-align:center}.hero small{letter-spacing:.12em;text-transform:uppercase;font-weight:900;color:#ffffffb8}.hero h1{font:700 clamp(2.3rem,7vw,4.5rem) Georgia,serif;line-height:1.08;max-width:900px;margin:.5rem auto}.wrap{max-width:820px;margin:auto;padding:2rem 1rem 5rem}.paper{background:#fff;border:1px solid #e2dfd6;border-radius:16px;padding:clamp(1.3rem,5vw,3rem);box-shadow:0 12px 30px #0000000a}.meta{font-size:.82rem;color:#666;border-bottom:1px solid #eee;padding-bottom:1rem}.story p{font:1.1rem/1.85 Georgia,serif}.share{border-top:1px solid #eee;margin-top:2rem;padding-top:1.4rem}.buttons{display:flex;gap:.55rem;flex-wrap:wrap}.buttons a,.buttons button{border:0;border-radius:8px;padding:.72rem .9rem;font-weight:900;text-decoration:none;cursor:pointer;background:#002d62;color:#fff}.buttons .wa{background:#128c4a}.buttons .fb{background:#1877f2}.buttons .x{background:#111}.cta{margin-top:2rem;background:#eef4fb;border-radius:12px;padding:1.2rem}.cta a{display:inline-block;margin-top:.6rem;color:#fff;background:#ce1126;padding:.65rem .85rem;border-radius:8px;text-decoration:none;font-weight:900}.note{font-size:.74rem;color:#777;margin-top:2rem}</style></head><body><nav><a href="/">🇵🇷 Planeta Boricua</a><a href="/pueblos/${encodeURIComponent(item.town)}/">← ${esc(town)}</a></nav><header class="hero"><small>Historias de mi pueblo · ${esc(town)}</small><h1>${esc(item.title)}</h1></header><main class="wrap"><article class="paper"><div class="meta">Por <strong>${esc(item.name)}</strong>${place}</div><div class="story">${paragraphs}</div><section class="share"><h2>Comparte esta historia 🇵🇷</h2><p>Pásasela a familiares y amigos de ${esc(town)}. Quizás ellos recuerdan algo diferente o tienen su propia historia que contar.</p><div class="buttons"><a class="wa" target="_blank" rel="noopener" href="https://wa.me/?text=${share}">WhatsApp</a><a class="fb" target="_blank" rel="noopener" href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(canonical)}">Facebook</a><a class="x" target="_blank" rel="noopener" href="https://twitter.com/intent/tweet?text=${encodeURIComponent(`${item.name} compartió una historia de ${town} 🇵🇷`)}&url=${encodeURIComponent(canonical)}">X</a><button id="copy">Copiar invitación</button></div><p id="status"></p></section><div class="cta"><strong>🇵🇷 Que siga la historia de ${esc(town)}</strong><br>¿Conoces a alguien de ${esc(town)} que tenga un recuerdo que contar? Comparte esta página e invítalo a dejar su propia historia.<br><a href="/pueblos/${encodeURIComponent(item.town)}/#historias">Contar una historia de ${esc(town)} →</a></div><p class="note">Las historias representan recuerdos y aportaciones de sus autores. Planeta Boricua puede editar formato, moderar o retirar contenido que viole sus normas.</p></article></main><script>document.getElementById('copy').onclick=async()=>{const text=${JSON.stringify(inviteText)};try{await navigator.clipboard.writeText(text);document.getElementById('status').textContent='Invitación copiada. Ya puedes pegarla donde quieras.'}catch(_){prompt('Copia esta invitación:',text)}}</script></body></html>`;
}
...
module.exports = function registerPBStories(app) {
  ...
  app.post('/api/pb-stories/:town', limiter, require('express').json({limit:'40kb'}), async (req,res) => {
    ...
    const contactPattern=/(https?:\/\/|www\.|\b[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}\b|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4})/i;
    const sensitive=/\b(?:ssn|social security|credit card|tarjeta de cr[eé]dito|password|contrase[nñ]a)\b/i.test(story);
    if (contactPattern.test(story) || sensitive) return res.status(400).json({ok:false,error:'No pudimos enviar tu historia. Revísala y asegúrate de no incluir enlaces, direcciones web, números de teléfono ni correos electrónicos. Puedes mencionar negocios y lugares por su nombre.'});
