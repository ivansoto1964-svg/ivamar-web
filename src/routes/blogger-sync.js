...
async function syncBlogger() {
  try {
    ...
    for (const entry of entries) {
      ...
      const featuredCategory = cats.find(c => {
        const normalized = c.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
        return normalized === "lo mas reciente" || normalized === "ultima hora";
      });
      const category = featuredCategory && featuredCategory.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase() === "ultima hora"
        ? "Lo más reciente"
        : (featuredCategory || (cats.length > 0 ? cats[0] : "Cultura Boricua"));
